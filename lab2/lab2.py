            ...
            with open(filename, "r") as file:
                ...
                while True:
                    ...
                    for _ in range(cls._char_height):
                        ...
                        if '-' in line:
                            raise ValueError(f"файл шрифтов недействителен. вот корректный список {cls._font.keys()}")
                        cls._font[char].append(line)
        except Exception as e:
            print(f"ошибка загрузки файла с шрифтами: {e}")
    ...
    def print_(cls, text: str, color: Color, position: tuple[int, int], symbol: str, background_color: Color = Color.BLACK) -> None:
        ...
        for char in text:
            if char not in cls._font:
                raise ValueError(f"{char} нет в файле")
...
if __name__ == "__main__":
    ...
    Printer.print_("ABCDEFGHS", Color.RED, (5, 2), "#", background_color=Color.TRANSPARENT)
    Printer.load_font(filename="font7.txt")
    with Printer(Color.GREEN, (0, 10), "@", background_color=Color.BLACK) as printer:
        printer.print("SUPER CLASS")
        printer.print("ABCDEFGHS")
